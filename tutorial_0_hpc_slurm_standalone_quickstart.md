...
**無需 `git clone` 本儲存庫，亦無需安裝任何客製化 Agent Skill**。您可以直接複製以下 Prompt 發送給任何具備 Terminal 執行權限的 AI Agent（例如 ChatGPT、Claude、Cursor、Antigravity 等），自動完成 HPC 資源查詢、計畫授權驗證與生物資訊分析作業派送。
...
隨後依序複製本指南中的 **Prompt 1 至 Prompt 4** 發送給 AI Agent，AI 就會自動完成從資源盤點、寫入測試檔案到成功派送 Slurm 作業的全部操作！

---

## 📋 快速導覽清單

| 階段 | 步驟 | 任務目標 | 核心原生指令 | 說明 |
| :--- | :--- | :--- | :--- | :--- |
| **一、環境與資源查詢** | **Prompt 1** | 查詢可使用的 Slurm Partition 資源與硬體限制 | `sinfo` / `scontrol` | 全面盤點節點、CPU、記憶體與 MaxTime |
| | **Prompt 2** | 使用 `wallet` 指令列出可用計畫代碼與額度 | `wallet` | 查詢個人名下計畫代碼與點數餘額 |
| | **Prompt 3** | 確認特定計畫代碼 (`MST109178`) 與 NGS partition 權限 | `sacctmgr` / `scontrol` | 透過調度器驗證專屬生醫計畫權限 |
| **二、生物資訊分析派送** | **Prompt 4** | 自動產生測試資料、編寫 Python 統計腳本並派送 Slurm Job | `sbatch` / Python / `ngs8g` | 端到端自動化 FASTQ 統計與作業派送 |
...
* **優勢與特點**：要求 AI Agent 搭配 `scontrol show partition` 查詢，能避免僅解析 `sinfo` 而遺漏各 Partition 的時限與 Allow/DenyAccounts 詳細限制。
...
* **優勢與特點**：Prompt 保持極簡，Agent 執行後會自動彙整表格呈現計畫代碼、計畫名稱與可用點數。

---

### 📌 Prompt 3：確認特定計畫代碼 (MST109178)

```text
請幫我確認生醫計畫代碼 `MST109178` 是否具有 Slurm 作業派送權限：
1. 請執行 `sacctmgr -nP show assoc user="$USER" account="mst109178"` 確認是否具備 Slurm association 授權。
2. 請執行 `scontrol show partition ngs8g` 檢查該 Partition 的 AllowAccounts / DenyAccounts 政策，驗證 `MST109178` 能否派送至 `ngs8g`。
3. 此生醫計畫可能不出現在一般 `wallet` 清單中，請同時說明檢查結果。
```

#### 📖 說明與原理：
* **用途**：驗證特定計畫（例如專屬的生醫計畫 `MST109178`）是否擁有調度器層級的派送授權，避開單靠 `wallet` 查詢產生的誤判。
* **無 Skill 原理**：完全移除對第三方擴充技能的依賴，明確指示 AI 使用 Slurm 原生資料庫查詢指令 `sacctmgr` 與節點控制指令 `scontrol`。
* **優勢與特點**：給予 AI 明確的指令執行路徑，即使在沒有安裝專用 Skill 的環境下，AI 也能精確驗證 Account 與 Partition 之間的存取權限。
...
3. 在 `script/` 目錄下撰寫 Slurm 提交腳本，分割區設為 `ngs8g`，將日誌寫入 `logs/` 目錄；不要把計畫代碼硬編碼在版本控制腳本內。
4. 請先確認 `logs/` 目錄已建立，並驗證 `MST109178` 與 `ngs8g` 權限後，使用 `sbatch --account="MST109178"` 派送作業，並回報 Job ID 與成果檢視方式。
```
...
* **優勢與特點**：確保作業能在無 Skill 環境下 100% 成功提交，同時維持安全性規範（如 `logs/` 目錄前置檢查、不寫死帳號在 Slurm 腳本內等）。
